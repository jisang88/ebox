...
<div id="page-wrapper">
	...
	<script type="text/javascript">
		var URL_SCREEN = {
			WRITE : "${pageContext.request.contextPath }/admin/screen/write",
			LIST : "${pageContext.request.contextPath }/admin/screen/list",
			DELETE : "${pageContext.request.contextPath }/admin/screen/delete",
			UPDATE : "${pageContext.request.contextPath }/admin/screen/update",
			READ : "${pageContext.request.contextPath }/admin/screen/read"
		}
							...
							<table class="table  table-striped">
								...
								<thead class="bg-inverse text-inverse ">
									<tr>
										...
										<th class="thead-col-2">영화제목(한글/영어)</th>
										<th class="thead-col-3">감독/개봉날짜</th>
										<th class="thead-col-4">스크린모드</th>
										<th class="thead-col-5">구입 날짜</th>
										<th class="thead-col-6">금액</th>
										<th class="thead-col-7">상영 시작 날짜</th>
										<th class="thead-col-8">상영 종료 날짜</th>
								...
								<tbody>
									...
									<c:forEach var="screen" items="${screenList}">
										...
										<tr>
											...
											<td class="tbody-col-1"><label class="waves-effect"><input type="checkbox" class="chk-box" name="aNo" value="${screen.scrNo}"></label></td>
											<td class="tbody-col-2"><a href="#">${!empty screen.movie.mNm? screen.movie.mNm:'-'}</a></td>
											<td class="tbody-col-3">${!empty screen.movie.mDirector? screen.movie.mDirector:'-'}</td>
											<td class="tbody-col-4">${!empty screen.scrType? screen.scrType.toUpperCase() :'-'}</td>

											<td class="tbody-col-5">
												<!--  
											--> <c:choose>
													<c:when test="${empty screen.scrBuyDt}">-</c:when>
													<c:otherwise>
														<fmt:formatDate value="${screen.scrBuyDt}" pattern="yyyy-MM-dd" />
													</c:otherwise>
												</c:choose> <!--  
											-->
											</td>

											<td class="tbody-col-6">
												<!--  
											--> <c:choose>
													<c:when test="${empty screen.scrPrice}">-</c:when>
													<c:otherwise>
														<fmt:formatNumber value="${screen.scrPrice}" type="currency" currencySymbol="￦" />
													</c:otherwise>
												</c:choose> <!--  
											-->
											</td>
											...
											<td class="tbody-col-7">
												<!--  
											--> <c:choose>
													<c:when test="${empty screen.scrSdate}">-</c:when>
													<c:otherwise>
														<fmt:formatDate value="${screen.scrSdate}" pattern="yyyy-MM-dd" />
													</c:otherwise>
												</c:choose> <!--  
											-->
											</td>
											...
											<td class="tbody-col-8">
												<!--  
											--> <c:choose>
													<c:when test="${empty screen.scrEdate}">-</c:when>
													<c:otherwise>
														<fmt:formatDate value="${screen.scrEdate}" pattern="yyyy-MM-dd" />
													</c:otherwise>
												</c:choose> <!--  
											-->
											...
											<td class="tbody-col-9">
												<button class="btn btn-warning btn-xs waves-effect waves-light btn-update-in-row" data-pk="${screen.scrNo}" ondblclick="return false;">수정</button>
												<button class="btn btn-danger btn-xs waves-effect waves-light btn-delete-in-row" data-pk="${screen.scrNo}" ondblclick="return false;">삭제</button>
											</td>
